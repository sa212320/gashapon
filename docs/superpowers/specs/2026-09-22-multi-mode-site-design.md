...
無框架、無打包。three.js **自己託管在 `vendor/three.module.min.js`**,各模式用 importmap 指過去。

> 原本寫的是「只有 3D 扭蛋機用、走 CDN」。兩點都改了:一番賞的開籤演出先用上了它,
> 而且使用對象是小學生、可能在學校網路 —— CDN 被擋等於整個模式打不開,所以改成自己託管。
> 已經進 repo,後續模式用它的**額外體積是零**。
...
three.js 場景:透視跑道、跟拍鏡頭、貼地的彩色軌跡帶。參考對象 gachago 的 GhostLeg
本身就是 WebGL 3D 場景,不是 2D 圖。

**公平性來自兩端各自洗牌,不是來自橫槓密度** —— 這是整節最重要的一句。
起始位置隨機、終點獎項隨機,所以不論梯子連得多亂多不亂,每個人拿到每個獎的機率都均勻。
梯子在數學上只是一個雙射,它是**演出**。

流程:

1. `lineup` = 把 `players` 洗牌後放上車道。畫面上的順序跟設定清單無關
2. `slots` = `expand(prizes)` 展開後**隨機取樣** `players.length` 份,再洗牌。
   **`slots.length` 恆等於 `players.length`**:不足時補「銘謝惠顧」(算出來的,不存)
3. 隨機生成橫槓。列數只由「一次看得完多久」決定(12～16),
   橫槓總數只影響好不好看 —— **不要把這兩個數字綁在一起**
4. **純函式 `walk(ladder, startIndex) → endIndex`** 算出每個 player 的終點 —— 核心演算法,必測
5. 演出:開場俯視掃過整條梯子約 1.5 秒 → 降到隊伍後方 → 跟著前緣跑 →
   最後一段鏡頭退開,讓落後的人也一起入鏡抵達
6. 抵達後疊一張可捲動的結果清單(誰 → 什麼獎)

**人數**:上限 40。車道寬度固定到某個人數為止,再多才壓縮車道、鏡頭只退一點。
**顏色**:20 色調色盤,自動指派、用完才重複。
**名字才是識別** —— 超過 20 人顏色會重複,而且約 8% 的男生有紅綠色覺辨異,
從一開始就分不出顏色。顏色是加分,不是識別。
**總長固定約 10 秒**,不管 4 人還是 40 人;不做快轉鍵。
**軌跡帶不消失**,跑完看得到「起點 → 一路轉彎 → 我的獎」那張完整的圖。
