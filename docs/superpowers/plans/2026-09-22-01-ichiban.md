...
import {
  ...
  buildTickets, remainingTickets, drawTicket, refillSetup,
...
test('抽一張會消耗一張,而且不動到獎項的 count', () => {
  ...
  assert.equal(remainingTickets(setup), 3, 'drawTicket 不該就地改動 setup');
});
...
test('refillSetup 把籤全部放回去,不動 prizes', () => {
  ...
  assert.equal(remainingTickets(refilled), 2);
...
export function remainingTickets(setup) {
  return setup.tickets.filter(t => !t.drawn).length;
}
