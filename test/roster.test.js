...
// --- entriesChanged:Date / RegExp 這類「沒有自身可列舉屬性」的物件不能走列舉 key 那條路 ---

test('entriesChanged:不同的 Date 算變了', () => {
  assert.equal(entriesChanged(
    [{ id: 'a', count: 1, at: new Date('2024-01-01T00:00:00Z') }],
    [{ id: 'a', count: 1, at: new Date('2024-06-01T00:00:00Z') }]), true);
});

test('entriesChanged:同一個時間點的 Date 算沒變', () => {
  assert.equal(entriesChanged(
    [{ id: 'a', count: 1, at: new Date('2024-01-01T00:00:00Z') }],
    [{ id: 'a', count: 1, at: new Date('2024-01-01T00:00:00Z') }]), false);
});

test('entriesChanged:Date 跟長得像它的普通物件不算相等', () => {
  assert.equal(entriesChanged(
    [{ id: 'a', count: 1, at: new Date(0) }],
    [{ id: 'a', count: 1, at: {} }]), true);
});

test('entriesChanged:不同的 RegExp 算變了', () => {
  assert.equal(entriesChanged(
    [{ id: 'a', count: 1, re: /abc/g }],
    [{ id: 'a', count: 1, re: /abc/i }]), true);
});
...
test('entriesChanged:pattern 與 flags 都一樣的 RegExp 算沒變', () => {
  assert.equal(entriesChanged(
    [{ id: 'a', count: 1, re: /abc/gi }],
    [{ id: 'a', count: 1, re: /abc/gi }]), false);
});

test('entriesChanged:兩個 Invalid Date 算沒變,且不會跟數字 NaN 混在一起', () => {
  assert.equal(entriesChanged(
    [{ id: 'a', count: 1, at: new Date('not-a-date') }],
    [{ id: 'a', count: 1, at: new Date('also-not-a-date') }]), false);
  assert.equal(entriesChanged(
    [{ id: 'a', count: 1, at: new Date('not-a-date') }],
    [{ id: 'a', count: 1, at: NaN }]), true);
});
